...
export default function ProductsByCategoryScreen({navigation}){
    ...
    useEffect(() => {
        api.post("/getProductByType",{productType:categoryName})
            .then(res => {
                let products = [];
                for (let i = 0; i < res.data.instantProduct.length; i++) {

                    const currProduct={ id: res.data.instantProduct[i].productId, title: res.data.instantProduct[i].title, cost:res.data.instantProduct[i].price, productFilters: res.data.instantProduct[i].productFilters,
                        location: res.data.instantProduct[i].location};

                    products.push(currProduct);
                }
                setDataJson(products);
            })
    }, [isLoadingForProducts]);
                    ...
                    <View style={styles.searchBox}>
                        <View style={{flex: 9}}>
                            <TextInput
                                ...
                                placeholder={"Search on " + categoryName}
                            />
                        </View>
